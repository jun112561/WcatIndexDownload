...
using WcatIndexDownload.Properties;
...
namespace WcatIndexDownload
{
    public partial class Form1 : Form
    {
        ...
        {
#if DEBUG
            txt_Save.Text = Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory) + "\\test\\";
            SFD.InitialDirectory = txt_Save.Text;
#else
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if (ramCounter.NextValue() >= 2048f) GC.Collect();
        }
        ...
        private void RunDownload()
        {
            ...
            if (rdb_ST.Checked) Url = "https://img.wcproject.so-net.tw/assets/";
            else if (rdb_SJ.Checked) Url = "https://i-cf.wcat.colopl.jp/assets/";
            else if (rdb_tennis.Checked) Url = "https://i-cf.tennis.colopl.jp/asset_bundle/";
            else Url = txt_URL.Text;

            if (!rdb_Custom.Checked && RB_2018.Checked) Url += "2018/";
            else if (!rdb_Custom.Checked && RB_2020.Checked) Url += "2020/";
            else if (!rdb_Custom.Checked && RB_2022.Checked) Url += "2022/";

            if (rdb_tennis.Checked && RB_A.Checked) Url += "android/2.1.0/";
            else if (rdb_tennis.Checked && RB_I.Checked) Url += "ios/2.1.0/";
            ...
            {
                if (string.IsNullOrWhiteSpace(fileName) || fileName.StartsWith("'") || !fileName.ComparisonFileName()) DelListBox(fileName.ToString(), lib_File);
                else
                {
            ...
            list.ForEach(delegate (DownloadInfo item)
            {
                ...
                while (!item.IsDone)
                {
                    if (!isWorking) { item.WC?.CancelAsync(); break; }
                            ...
                            switch (OR.type)
                            {
                                case ClassIDType.Texture2D:
                                    ...
                                    if (image != null)
                                    {
                                        ...
                                        {
                                            if (!fileName.Contains("std") && !fileName.EndsWith("w_png") && (image.Width == 1024 && image.Height == 1024)) image.Mutate((x) => x.Resize(1024, 1331));
                                        }
                                        else if (fileName.StartsWith("Location"))
                                        {
                                            if (image.Width == 512 && image.Height == 512) image.Mutate((x) => x.Resize(768, 512));
                                            else if (image.Width == 1024 && image.Height == 1024) image.Mutate((x) => x.Resize(1536, 1024));
                                        }
                                        else if (fileName.Contains("loginBonus_bg") && (image.Width == 1024 && image.Height == 1024)) image.Mutate((x) => x.Resize(1024, 1536));

                                        using (image)
                                        {
                                            using (var file = File.OpenWrite(SavePath + fileName + ".png"))
                                            {
                                ...
                                case ClassIDType.AudioClip:
                                    ...
                                    if (converter.IsSupport)
                                    {
                                        ...
                                        if (buffer != null)
                                            File.WriteAllBytes(SavePath + fileName + ".wav", buffer);
                                    }
                                    else
                                    {
                                        File.WriteAllBytes(SavePath + fileName + converter.GetExtensionName(), m_AudioData);
                                    }
